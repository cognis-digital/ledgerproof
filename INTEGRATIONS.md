...
**ledgerproof** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `ledgerproof` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
ledgerproof ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
ledgerproof ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
ledgerproof ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `ledgerproof` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="ledgerproof") for rec in records]   # records = your JSON output
